...
async function testYearFieldFix() {
  console.log('🔍 测试使用年度字段修复后的逻辑...');
  console.log();
  ...
  try {
    ...
    console.log('📊 杨治源社保数据:');
    yangData?.forEach((record, index) => {
      console.log(`\n记录 ${index + 1}:`);
      console.log(`  开始时间: ${record.开始时间}`);
      console.log(`  年度字段: ${record.年度}`);
      console.log(`  险种类型: ${record.险种类型}`);
      
      // 测试修复后的年度逻辑
      const calculatedYear = getSocialInsuranceYear(record.开始时间);
      const finalYear = record.年度 ? `${record.年度}年度` : calculatedYear;
      
      console.log(`  计算的年度: ${calculatedYear}`);
      console.log(`  最终使用的年度: ${finalYear}`);
    });

    // 修正后的四个基本险种（个人缴纳）
    const requiredInsuranceTypes = ['养老保险', '医疗保险', '失业保险', '公积金'];
    console.log('\n✅ 应缴险种:', requiredInsuranceTypes.join('、'));
    console.log();

    // 按年度分组杨治源的社保数据（使用修复后的逻辑）
    const yearlyData = {};
    const employeeNames = {};
    ...
    yangData?.forEach((record) => {
      ...
      // 使用数据库中的年度字段，如果没有则计算社保年度（修复后的逻辑）
      const year = record.年度 ? `${record.年度}年度` : getSocialInsuranceYear(startTime);
      
      console.log(`📋 处理记录: ${startTime} -> 年度字段: ${record.年度} -> 最终年度: ${year}, 险种: ${record.险种类型} -> ${insuranceType}`);
    ...
    console.log();
    console.log('🎯 修复验证结论:');
    console.log('- 修复前：使用开始时间计算年度，2022-07-01 -> 2022年度');
    console.log('- 修复后：优先使用数据库年度字段，年度字段=2023 -> 2023年度');
    console.log('- 杨治源现在应该正确归类到2023年度，只缴纳养老保险，缺3项');
